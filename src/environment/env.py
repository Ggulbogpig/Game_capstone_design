...
from smb import *
from src.utils.mymath import easy_post_process
...
class OffRewardGenerationEnv(gym.Env):
    ...
    def step(self, action: np.ndarray):
        ...
        if done:
            ...
            if self.mario_proxy:
                ...
                #  playable 상태를 직접 확인
                print("=== playable 상태 ===")
                for idx, item in enumerate(simlt_res[1:], start=1):  # simlt_res[0]은 건너뛰는 걸로 보임
                 print(f"Segment {idx}: playable={item['playable']}")
    ...
    def reset(self):
        ...
        if self.backup_latvec is not None and self.backup_onehot_seg is not None:  # ✅ 추가
            for latvec, onehot_seg in zip(self.backup_latvec, self.backup_onehot_seg):
                self.latvec_archive.push(latvec)
                self.segs.append(MarioLevel.from_one_hot_arr(onehot_seg))
        else:
            # backup이 없을 땐 랜덤 초기화 또는 예외처리
            print("경고: backup_latvec이 없어서 랜덤 초기화")
...
class VecGenerationEnv(SubprocVecEnv):
    ...
    def step_async(self, actions: np.ndarray) -> None:
        with torch.no_grad():
            ...
            #추가가
            onehot_segs = [easy_post_process(seg) for seg in onehot_segs]
