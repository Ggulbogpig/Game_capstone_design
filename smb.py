...
if not jpype.isJVMStarted():
    jvm_path = os.path.abspath("C:/Program Files/Java/jdk-24/bin/server/jvm.dll")
    jar_path = os.path.abspath("C:/Users/hbsss/Downloads/MFEDRL-master_250601/MFEDRL-master/Mario-AI-Framework_agent0.jar")
...
class MarioProxy:
    ...
    def __init__(self):
         ...
         if not jpype.isJVMStarted():
            ...
            #     f"-Djava.class.path=C:/Users/hbsss/Downloads/MFEDRL-master_250601/MFEDRL-master/Mario-AI-Framework_agent0.jar", '-Xmx2g'
            # )
            ...
            # self.__proxy = jpype.JClass("MarioProxy")()
                ...
                "-ea",
                "-classpath",
                "C:/Users/hbsss/Downloads/MFEDRL-master_250601/MFEDRL-master"
                )

                
         self.__proxy = jpype.JClass("MarioProxy")()  

         jpype.JClass("MarioProxy")
...
#수정
    ...
    def __extract_res(jresult):
        completing_ratio_str = jresult.getCompletionPercentage()
        completing_ratio = float(completing_ratio_str)
        return {
            'status': str(jresult.getGameStatus().toString()),
            #'completing-ratio': float(jresult.getCompletionPercentage()),
            'completing-ratio': completing_ratio,
    ...
    def get_seg_infos(full_info, check_points=None):
        ...
        with open(log_file, "a") as f:
            f.write("=== get_seg_infos: restarts ===\n")
            f.write(str(restarts) + "\n")
            f.write("=== get_seg_infos: trace (첫 10개) ===\n")
            f.write(str(trace[:10]) + "\n")
    
        print("=== get_seg_infos: restarts ===")
        print(restarts)
        print("=== get_seg_infos: trace ===")
        print(trace[:10])
        ...
        with open(log_file, "a") as f:
            f.write("=== check_points ===\n")
            f.write(str(check_points) + "\n")
        ...
        with open(log_file, "a") as f:
            ...
            for idx, item in enumerate(res):
                f.write(f"Segment {idx}: playable={item['playable']}\n")
            f.write("=== get_seg_infos 끝 ===\n\n")
...
if __name__ == '__main__':
    #lvl = MarioLevel.from_file('levels/levels_test/lvl-1.txt')
    #lvl = MarioLevel.from_file('levels/original/mario-1-1.smblv')
    #lvl = MarioLevel.from_file('exp_analysis/endless_gen/agent1/agent1Levels.smblvs')
    lvl = MarioLevel.from_file('exp_analysis/endless_gen/leveltest/agent1_level_1.txt')
